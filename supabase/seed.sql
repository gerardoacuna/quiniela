...
insert into public.riders (id, edition_id, pcs_slug, name, team, bib, status) values
  ...
  ('20000000-0000-4000-8000-000000000002', '00000000-0000-4000-8000-000000000001', 'juan-ayuso',        'Juan Ayuso',         'UAE Team Emirates', 2,  'active'),
  ('20000000-0000-4000-8000-000000000003', '00000000-0000-4000-8000-000000000001', 'remco-evenepoel',   'Remco Evenepoel',    'Soudal Quick-Step',  11, 'active'),
  ('20000000-0000-4000-8000-000000000004', '00000000-0000-4000-8000-000000000001', 'primoz-roglic',     'Primož Roglič',      'Red Bull - BORA',    21, 'active'),
  ('20000000-0000-4000-8000-000000000005', '00000000-0000-4000-8000-000000000001', 'filippo-ganna',     'Filippo Ganna',      'INEOS Grenadiers',   31, 'active');
...
values
  ('30000000-0000-4000-8000-000000000001', 'dev-admin@example.com',  crypt('devpass123', gen_salt('bf')), now(), 'authenticated', 'authenticated', '00000000-0000-0000-0000-000000000000', '{"provider":"email","providers":["email"]}'::jsonb, '{}'::jsonb, now(), now(), '', '', '', '', '', '', '', ''),
  ('30000000-0000-4000-8000-000000000002', 'dev-player@example.com', crypt('devpass123', gen_salt('bf')), now(), 'authenticated', 'authenticated', '00000000-0000-0000-0000-000000000000', '{"provider":"email","providers":["email"]}'::jsonb, '{}'::jsonb, now(), now(), '', '', '', '', '', '', '', '')
on conflict (id) do nothing;

insert into public.profiles (id, display_name, role, email) values
  ('30000000-0000-4000-8000-000000000001', 'Admin (dev)',  'admin',  'dev-admin@example.com'),
  ('30000000-0000-4000-8000-000000000002', 'Player (dev)', 'player', 'dev-player@example.com')
on conflict (id) do nothing;

insert into public.invites (code, created_by, email, expires_at) values
  ('dev-invite-0001', '30000000-0000-4000-8000-000000000001', 'dev-invitee@example.com', now() + interval '7 days')
...
insert into public.stage_results (stage_id, position, rider_id, status) values
  ('10000000-0000-4000-8000-000000000001', 1, '20000000-0000-4000-8000-000000000001', 'published'),
  ('10000000-0000-4000-8000-000000000001', 2, '20000000-0000-4000-8000-000000000002', 'published'),
  ('10000000-0000-4000-8000-000000000001', 3, '20000000-0000-4000-8000-000000000003', 'published'),
  ('10000000-0000-4000-8000-000000000001', 4, '20000000-0000-4000-8000-000000000004', 'published'),
  ('10000000-0000-4000-8000-000000000001', 5, '20000000-0000-4000-8000-000000000005', 'published')

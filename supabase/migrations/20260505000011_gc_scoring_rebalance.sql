-- GC scoring rebalance for Giro 2026:
--   exact-position match 30 → 50
--   in-podium wrong-position 10 → 25
-- No backfill needed (no published GC yet); leaderboard_view recomputes on every read.
...
with
  ...
  gc_scoring as (
    select
      ...
      sum(
        case
          when gp.rider_id = fc_exact.rider_id then 50
          when gp.rider_id in (
